...
//      ShowBoard();
     ...
     { 
      ...
      if (gamesound) Play_MP3(AUDIO_FILENR,false,33*gamesound);
       ...
       if ( firstmove ) 
       {
        ...
        if (move != -1) {
            ...
            if ( A_wirfStein )
              while ( wirfStein(move, SPIELER_A));
            ShowBoard();         
      ...
      if (gamesound) Play_MP3(AUDIO_FILENR,false,33*gamesound);
        ...
        {
#ifdef DEBUG_GAME
          Serial.println(F("No move possible"));
#endif
        }
     }
     l_01_spieler = 1;
...
void abMinimax(int maximizeOrMinimize, int8_t spieler, uint8_t depth, int8_t board[][WIDTH], int8_t* move, int* score )
{
    ...
    for (column=0; column<WIDTH; column++) {
        ...
        if (scoreInner == SPIELER_A_WINS || scoreInner == SPIELER_B_WINS)
            scoreInner -= depth * (int)spieler;
            
        if (depth == g_maxDepth-1 && B_wirfStein )
          B_wirfStein = wirfStein(g_4g_eingeworfen, spieler);
        ...
        if (depth == g_maxLDepth && !B_wirfStein )
        {
          ...
          {
            AUDIO_FILENR = ANSAGEBASE + 196 ;   // ich überlege
            if (gamesound) Play_MP3(AUDIO_FILENR,false,33*gamesound);
          }
#endif    
        if (depth == g_maxDepth)
        ...
        if (maximizeOrMinimize) {
            if (scoreInner==bestScore && random(0,2) == 1 ) {
...
void markiereGewinnSteine()
{
  ...
  for(uint8_t x=0;x<FIELD_WIDTH;x++)
  {
    ...
    { 
      ...
      {
        wert = 0;
        for( uint8_t i=0;i<4;i++ )        // Jeweils die nächsten 4 Nachbarn
        {
...
void Spieldrucken(){
  ...
  Serial.print(" ");
  for (j = 0; j < WIDTH ; j++) Serial.printf(" %i",j + 1);
  Serial.println(" ");
}
*/
