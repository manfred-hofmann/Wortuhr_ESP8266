...
void LedDriver::show()
{
  if ( mode != MODE_BLANK && (( mode == MODE_TIME && settings.mySettings.enable_bg_color == 1) || settings.mySettings.enable_bg_color == 2 ))
  {
    ...
    for ( uint8_t num = 0 ; num < 110; num++)
    {
      ...
      {
        strip->setPixelColor(pgm_read_byte(&ledMap[num]), red, green, blue);
      }
    }
  }
  strip->show();
...
//   https://andi-siess.de/rgb-to-color-temperature 
    ...
    strip->setPixelColor(pgm_read_byte(&ledMap[num]), red, green, blue, white / whiteboost);
#else
// Ohne Einstellen der Farbtemperatur:
    ...
    strip->setPixelColor(pgm_read_byte(&ledMap[num]), red-white, green-white, blue-white, white);
#endif
